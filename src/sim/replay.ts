...
export function setupWorld(s: GameSetup): World {
  return newGame(s.map, s.mode, { allies: s.allies, diff: s.diff, seed: s.seed, ai: s.ai, races: s.races });
}
...
export function recordReplay(w: World, s: GameSetup): Replay {
  return {
    ...
    allies: s.allies.slice(), diff: s.diff, ai: s.ai.slice(), races: s.races.slice(), commands: w.log.map((c) => JSON.parse(JSON.stringify(c)) as Command), ticks: w.tick,
  };
}
...
export function runReplay(r: Replay, untilTick = r.ticks): World {
  ...
  map.name = r.mapName;
  const w = newGame(map, r.mode, { allies: r.allies, diff: r.diff, seed: r.seed, ai: r.ai, races: r.races });
