...
function loop(ts: number): void {
  ...
  if (w) {
    ...
    drawWorld(app.ctx, app.bg, w, { drag: app.drag, alpha: app.running && !app.paused ? Math.min(1, acc / DT) : 1, selection: app.selection, paused: app.paused, viewer: app.ctl, hover: app.hover, cam: app.cam, dpr: app.dpr, overlay: app.overlay, damageNumbers: app.settings.damageNumbers, shake, ghost: app.tool === 'power' && app.power != null && app.mouse ? { x: app.cam.x + app.mouse.x / app.cam.zoom, y: app.cam.y + app.mouse.y / app.cam.zoom, r: POWERS[app.power].r } : app.tool === 'cheat' && app.cheatTool?.r && app.mouse ? { x: app.cam.x + app.mouse.x / app.cam.zoom, y: app.cam.y + app.mouse.y / app.cam.zoom, r: app.cheatTool.r } : null, place: placePreview(app) });
