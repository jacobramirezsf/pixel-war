...
test('gate is two tiles, locked by default, and passes its owner without damage', () => {
  ...
  for (let i = 0; i < 4; i++) place(w, 0, 'kni', 40 + i * 12, 150);
  place(w, 1, 'inf', 75, 20);
...
test('gate auto-orients vertical between walls', () => {
  ...
  assert.equal(w.blds.find((b) => b.kind === 'gate')?.dir, 'v');
});
...
test('sandbox replay and mirror keep gates', () => {
  ...
  act(w, 0, { type: 'toEdit', payload: null });
  act(w, 1, { type: 'mirror', payload: null });
...
function bot(w: World, t: number): void {
  if (t < 25) {
    if (w.slots[0].gold >= 35) buy(w, 0, t % 2 < 1 ? 'shd' : 'arc');
    if (every(t, 8)) {
      const m = w.mines[((t / 8) | 0) % Math.max(1, w.mines.length)];
      if (m) moveAll(w, 0, m.x, m.y);
    }
    return;
  }
  const mix = t < 90 ? ['shd', 'xbw', 'arc', 'med'] : ['mor', 'shd', 'snp', 'med', 'xbw', 'shd'];
  const k = mix[((t * 2) | 0) % mix.length] as keyof typeof TYPES;
  if (w.slots[0].gold >= TYPES[k].cost) buy(w, 0, k);
  if (every(t, 15) && t > 100) chargeAll(w, 0);
}
...
test('skirmish is winnable by a competent bot and not by a blind rush', () => {
  ...
  for (const n of ['Crossroads', 'Riverlands', 'Highlands', 'Arena'])
    for (let seed = 1; seed <= 3; seed++) {
      const w = game('skirmish', n, { seed });
      run(w, 480, (tt) => bot(w, tt));
      games++;
      if (w.over === 'win') wins++;
    }
