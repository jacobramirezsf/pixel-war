...
test('population caps the army and a city unlocks advanced units', () => {
  ...
  assert.ok(!act(w, 0, { type: 'buy', payload: { unit: 'gnt' } }), 'giant needs a city');
  let bought = 0;
  while (act(w, 0, { type: 'buy', payload: { unit: 'inf' } })) bought++;
  ...
  assert.equal(cap.tier, 'fortress');
  run(w, TIERS.fortress.buildT + 1);
  assert.ok(act(w, 0, { type: 'upgrade', payload: { id: cap.id } }));
  assert.equal(cap.tier, 'city');
  run(w, TIERS.city.buildT + 1);
  assert.ok(hasCity(w, 0));
  w.slots[0].queue.length = 0;
  for (const u of w.units) if (u.team === 0) u.hp = 0;
  ticks(w, 2);
  assert.ok(act(w, 0, { type: 'buy', payload: { unit: 'gnt' } }), 'giant with a city');
});
...
test('walls cost materials, and materials come from the land', () => {
  ...
  w.slots[0].mat = 0;
  assert.ok(!act(w, 0, { type: 'build', payload: { x: home.x, y: home.y - 40, bld: 'wal' } }), 'no materials');
  w.slots[0].mat = 100;
  assert.ok(act(w, 0, { type: 'build', payload: { x: home.x, y: home.y - 40, bld: 'wal' } }));
